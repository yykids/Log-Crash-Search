...
상품을 활성화하고 나면 Log & Crash Search 용 AppKey가 생성됩니다. 이 AppKey가 있어야 로그 전송이 가능합니다. AppKey값을 확인합니다.
...
|검색 쿼리 시간 조건|	검색 쿼리의 시간 조건을 설정할 수 있습니다|
|로그 검색 결과 > 차트뷰|	로그 검색 결과를 막대 그래프로 출력하며, 클릭하면 오른쪽 단위(ex: 1Bar=1hour)에 맞는 기간을 기준으로 출력합니다|
|필드 검색| - host : 프로젝트에 속하는 호스트를 선택하는데 한가지 이상의 호스트 선택일 경우 쿼리를 사용합니다. <br/> - logLevel : 로그 레벨 별 출력이 가능합니다. (FATAL / ERROR / WARN / INFO / DEBUG) <br/> - logSource : 사용자가 설정한 [로그 소스]로 구분합니다. <br/> - logType : 사용자가 설정한 [프로젝트 이름]으로 구분합니다. <br/> - projectVersion : 사용자가 설정한 [프로젝트 버전]으로 구분합니다. <br/> - 커스텀 필드 : Settings > 검색 필드에서 등록한 커스텀 필드를 추가하여 사용할 수 있습니다.|
...
|필터 조건|	상태 – 크래시 이슈 처리 상태 <br/> 최근 – 최근 N일간 수집된 크래시 로그 조회 필터 <br/> 앱 버전 – 앱 버전 <br/> 태그 – 태그 <br/> 최소 발생수 – 크래시 발생 최소 N건 이상인 크래시 조회 필터 <br/> 예외종류 – 예외 종류 별 최근 필터 <br/> 위치 – 크래시 발생 지점 별 조회 필터 |
...
|트렌드 그래프| 다양한 트렌드 그래프를 볼 수 있습니다. <br/> - 크래시 카운트 <br/> - 사용자 수 <br/> - 모든 에러 통계 <br/> - 미해결 에러 통계 <br/> - 앱 버전 별 이슈 통계 <br/> - OS 버전 별 이슈 통계 <br/> - 디바이스 별 이슈 통계 <br/> - 국가 별 이슈 통계|
...
|시간 조건| 사용자 별 크래시 조회 시간 조건을 설정할 수 있습니다.|
|앱 버전| 앱 버전 별로 필터링 합니다.|
...
|앱 버전|	앱 버전 별로 필터링 합니다|
|크래시 별 발생 빈도 파이 그래프|	크래시 별 발생 빈도를 보여줍니다|
|크래시 별 발생 빈도 순위 테이블|	크래시 별 발생 빈도 순위를 보여줍니다|
...
- 알람 시간을 기준으로 검색 기간 설정을 통한 검색이 가능하며 버전 별 검색 기능 또한 제공합니다.
...
|항목|	설명|
|---|---|
|필터 조건 | - 최근 : 최근 15분, 60분, 24시간, 48시간동안의 조건동안의 시간 별 조회 필터, 사용자 지정은 시작/종료 일자를 선택하여 조회(최대 48시간) <br> - 앱 버전 : 앱 버전 별 조회 필터 <br> - OS 버전 : OS 버전 별 조회 필터 <br> - 디바이스 : 디바이스명 입력 조회 필터(prefix 검색) <br> - 통신사 : 통신사 입력 조회 필터(prefix 검색) <br> - 국가 : 국가 별 조회 필터 <br> - URL : URL 별 조회 필터|
|현재 플랫폼|	플랫폼(iOS, Android)을 선택하여 필터링 검색할 수 있습니다.|
|차트| 디바이스명을 검색 조건으로 설정할 수 있습니다.|
|시간 조건| 지연시간과 에러율을 타임라인 차트로 표시합니다.|
|URL 목록| URL 별 지연시간과 에러율을 목록으로 출력합니다. 목록 해더를 클릭하여 정렬 순서를 변경할 수 있습니다.|

### 지도

국가 별 지연시간과 에러율을 지도로 표시 합니다.

- 필터 조건은 국가 필터가 없는 것을 제외하면 모니터링 화면과 동일합니다. 
- 지도 우측 상단의 [+][-] 버튼을 클릭하여 확대 / 축소 할 수 있습니다.
- 지도에 마우스를 드래그 하면 위치 이동이 가능합니다. 

###  URL 설정

모니터링 이 필요한 대상 URL을 설정 합니다. 

- 화면의 [추가],[선택항목 삭제] 버튼을 클릭하여, 모니터링 할 URL 목록을 관리 합니다. 
- URL을 추가/삭제하면 변경사항은 실시간으로 반영됩니다.
- 최근 변경일은 URL을 마지막으로 추가/삭제한 날짜 입니다.
...
|항목|	설명|
|---|---|
|필터 조건 | - 최근 : 최근 15분, 60분, 24시간, 48시간동안의 조건동안의 시간별 조회 필터, 사용자 지정은 시작/종료 일자를 선택하여 조회(최대 48시간) <br> - 앱 버전 : 앱 버전별 조회 필터 <br> - OS 버전 : OS 버전별 조회 필터 <br> - 디바이스 : 디바이스명 입력 조회 필터(prefix 검색) <br> - 통신사 : 통신사 입력 조회 필터(prefix 검색) <br> - 국가 : 국가별 조회 필터 <br> - URL : URL별 조회 필터|
|현재 플랫폼|	플랫폼(iOS, Android)을 선택하여 필터링 검색할 수 있습니다.|
|차트| 디바이스명을 검색 조건으로 설정할 수 있습니다.|
|시간 조건| 지연시간과 에러율을 타임라인 차트로 표시합니다.|
|URL 목록| URL별 지연시간과 에러율을 목록으로 출력합니다. 목록 해더를 클릭하여 정렬 순서를 변경할 수 있습니다.|

### 지도

국가별 지연시간과 에러율을 지도로 표시 합니다.
